...
# Removed spinner function - was causing crashes
...
info "PATH updated - venv tools have priority"
...
# Run build WITHOUT terminal corruption
info "Running build (this will take 2-5 minutes)..."
info "Build log: $BUILD_LOG"
echo

NCPU=$(sysctl -n hw.ncpu 2>/dev/null || nproc 2>/dev/null || echo 4)
info "Using $NCPU CPU cores"

# FIXED: Simple build without while-read loop that corrupts terminal
# Just run scons and capture output - no fancy progress display
scons -u -j$NCPU 2>&1 | tee "$BUILD_LOG"
